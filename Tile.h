...
public:
    Tile();
    Tile(bool walkable, const glm::vec<2, int>& pos,
         const std::array<std::string, g_layer_count>& textures);
    ...
    inline glm::vec2 real_position() const
    {
        return glm::vec2(m_position) * float(g_tile_size);
    }
    // DO NOT USE AS INDEX, ITS `float`
    inline sf::Vector2f sf_position() const
    {
