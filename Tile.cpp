...
bool is_same_texture_id(const glm::vec<2,int>& pos, const Cell* cell, const glm::vec<2,int>& my_pos)
{
    if (pos.x < 0 || pos.y < 0 || pos.x >= g_cell_size || pos.y >= g_cell_size)
        return false;
...
Tile::Tile(bool walkable, const glm::vec<2,int>& pos, const std::array<std::string, 4>& textures)
    : m_position(pos), 
      m_layers({ 
