...
ResourceManager::ResourceManager(stdfs::path resource_root_directory)
    : m_res_root(stdfs::absolute(resource_root_directory)), m_textures()
{
...
ProceduralTextureId
ResourceManager::make_procedural_texture(const std::vector<TextureId>& textures)
{
    sf::Image image;
    image.create(g_texture_size, g_texture_size);
    for (auto& texture_id : textures)
        image.copy(m_textures[texture_id].copyToImage(), 0, 0, sf::IntRect(0, 0, 0, 0),
                   true);
    sf::Texture texture;
...
ProceduralTextureId
ResourceManager::make_procedural_texture(const std::vector<Id>& textures)
{
    sf::Image image;
    image.create(g_texture_size, g_texture_size);
    for (const Id& texture_id : textures)
    {
        if (!texture_id.invalid_id())
            image.copy((*texture_id.texture()).copyToImage(), 0, 0,
...
void ResourceManager::load_textures()
{
    ...
    for (const auto& tex_file_path : stdfs::directory_iterator(m_res_root / "textures"))
    {
        ...
        if (!success)
            throw std::runtime_error("Texture " + tex_file_path.path().string() +
                                     " could not be loaded.");
...
void ResourceManager::load_vertex_shaders()
{
    ...
    for (const auto& vertex_file_path :
         stdfs::directory_iterator(m_res_root / "shaders" / "vertex"))
    {
        log(" -> loading " << vertex_file_path.path().stem() << "...");
        std::shared_ptr<sf::Shader> sh = std::make_shared<sf::Shader>();
        bool success =
            sh->loadFromFile(vertex_file_path.path().native(), sf::Shader::Type::Vertex);
        if (!success)
            throw std::runtime_error("Vertex shader " + vertex_file_path.path().string() +
                                     " could not be loaded.");
...
void ResourceManager::load_fragment_shaders()
{
    ...
    for (const auto& fragment_file_path :
         stdfs::directory_iterator(m_res_root / "shaders" / "fragment"))
    {
        log(" -> loading " << fragment_file_path.path().stem() << "...");
        std::shared_ptr<sf::Shader> sh = std::make_shared<sf::Shader>();
        bool success = sh->loadFromFile(fragment_file_path.path().native(),
                                        sf::Shader::Type::Fragment);
        if (!success)
            throw std::runtime_error("Fragment shader " +
                                     fragment_file_path.path().stem().string() +
                                     " could not be loaded.");
...
void ResourceManager::load_full_shaders()
{
    ...
    for (const auto& vert_file :
         stdfs::directory_iterator(m_res_root / "shaders" / "vertex"))
    {
        // If file with the same stem and .frag extension exists we can compile a full
        // shader.
        stdfs::path frag_file = m_res_root / "shaders" / "fragment" /
                                (vert_file.path().stem().string() + ".frag");
        if (stdfs::exists(frag_file))
        {
            std::shared_ptr<sf::Shader> sh = std::make_shared<sf::Shader>();
            bool success =
                sh->loadFromFile(vert_file.path().string(), frag_file.string());
            if (!success)
                throw std::runtime_error("Full shader " +
                                         vert_file.path().stem().string() +
                                         " could not be loaded.");
...
void ResourceManager::monitor_changes(ResourceManager* resman)
{
    ...
    for (const auto& file :
         stdfs::directory_iterator(resman->m_res_root / "shaders" / "vertex"))
        vertex_shaders[file.path().string()] = file.last_write_time();

    std::map<std::string, stdfs::file_time_type> fragment_shaders;
    for (const auto& file :
         stdfs::directory_iterator(resman->m_res_root / "shaders" / "fragment"))
        fragment_shaders[file.path().string()] = file.last_write_time();

    while (resman->is_alive)
    {
        std::this_thread::sleep_for(
            std::chrono::seconds(5)); // FIXME: Choose a sane interval.

        for (const auto& file :
             stdfs::directory_iterator(resman->m_res_root / "textures"))
        {
            ...
            if (now != textures[path.native()])
            {
                ...
                while (!g_renderer->render_mx.try_lock())
                    ;
                bool success =
                    resman->m_textures[resman->m_texture_ids[path.stem().native()]]
                        .loadFromFile(path.native());
                g_renderer->render_mx.unlock();
                if (!success)
                    throw std::runtime_error("Texture " + path.stem().string() +
             ...
             stdfs::directory_iterator(resman->m_res_root / "shaders" / "vertex"))
        {
            ...
            if (now != vertex_shaders[path.native()])
            {
                ...
                {
                    resman->m_vertex_shaders
                        [resman->m_vertex_shader_ids[path.stem().native()]] = sh;
                }

                stdfs::path frag_file = resman->m_res_root / "shaders" / "fragment" /
                                        (path.stem().string() + ".frag");
                if (stdfs::exists(frag_file))
                {
                    std::shared_ptr<sf::Shader> sh = std::make_shared<sf::Shader>();
                    ...
                    {
                        resman->m_full_shaders
                            [resman->m_full_shader_ids[path.stem().native()]] = sh;
                    }
                }
             ...
             stdfs::directory_iterator(resman->m_res_root / "shaders" / "fragment"))
        {
            ...
            if (now != fragment_shaders[path.native()])
            {
                ...
                std::shared_ptr<sf::Shader> sh = std::make_shared<sf::Shader>();
                bool success =
                    sh->loadFromFile(path.native(), sf::Shader::Type::Fragment);
                if (!success)
                {
                    log("Reloading failed.");
                }
                ...
                {
                    resman->m_fragment_shaders
                        [resman->m_fragment_shader_ids[path.stem().native()]] = sh;
                }

                stdfs::path other_file = resman->m_res_root / "shaders" / "vertex" /
                                         (path.stem().string() + ".vert");
