...
RenderId Renderer::submit(VoidPtrWrapper self, const Tile& tile, const std::uint32_t& walls)
{
    ...
    m_tiles.emplace(self.reinterpret_as<RenderId>(), TileRenderInfo(tile.sf_position(), 
                    varray, 
                    g_resource_manager->make_procedural_texture(ids), 
                    g_resource_manager->get_full_shader_id("simple")
                ));
    
    return self.reinterpret_as<RenderId>(); // FIXME: RenderId doesn't work with this way of batching.
}
...
void Renderer::render()
{
    ...
    g_resource_manager->get_full_shader(shader)->setUniform("player_size", g_world->player().sf_size());
    g_resource_manager->get_full_shader(shader)->setUniform("player_position", player_pos);
    g_resource_manager->get_full_shader(shader)->setUniform("view_position", m_window->mapPixelToCoords(sf::Mouse::getPosition()));
